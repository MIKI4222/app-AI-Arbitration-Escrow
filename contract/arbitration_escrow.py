...
class ArbitrationEscrow(gl.Contract):
    ...
    def create_deal(self, freelancer: str, description: str, amount: int) -> None:
        ...
        deal = {
            ...
            "dispute_opened_at_tx": len(self.deals),
        }
    ...
    def cancel_stalled_dispute(self, deal_id: int) -> None:
        """
        Bounded escape from the DISPUTED state.
        If the freelancer has not submitted counter-evidence, the client
        can cancel the dispute and recover their position. This prevents
        a malicious or inactive freelancer from permanently blocking the
        contract's state machine.
        Only the client is authorized to call this path.
        """
        deal = json.loads(self.deals[deal_id])
        ...
        deal["resolution_reasoning"] = (
            "Dispute cancelled by client: freelancer did not submit "
            "counter-evidence. Deal resolved in favour of the client by default."
        )
        self.deals[deal_id] = json.dumps(deal, sort_keys=True)
